...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        # Code teacher model (for computing base_ref_log_prob in multi-teacher mode)
    ...
    def compute_ref_log_prob(self, data: DataProto):
        # Check if entropy-aware distillation is enabled
    ...
    def compute_base_ref_log_prob(self, data: DataProto):
        """Compute log probabilities (and optionally entropy) using ref's base model.

        In multi-teacher mode, this is the code teacher. When entropy-aware distillation
        is enabled, also computes per-token entropy for token-level weighting.

        Args:
            data: DataProto containing ref_input_ids, ref_attention_mask, ref_position_ids, responses

        Returns:
            DataProto with base_ref_log_prob tensor (and base_ref_entropys if entropy-aware)
        """
        ...
        with self.ulysses_sharding_manager:
            ...
            if compute_teacher_entropy and base_ref_entropys is not None:
                tensors["base_ref_entropys"] = base_ref_entropys
            output = DataProto.from_dict(tensors=tensors)
    ...
    def has_base_models(self):
        """Check if code teacher model is available for multi-teacher distillation."""
        return self._has_base_ref_model
